...
novo_valor = st.number_input("Insira o valor anual da franquia", min_value=1.0, max_value=999999.0, value=1500.0, step=0.01)
processar = st.button("Processar")

if processar:
    dados_novo_valor = pd.DataFrame([[novo_valor]], columns=['FrqAnual'])
    previsto = modelo.predict(dados_novo_valor)
    st.header(f"Prveisão de custo inicial R$: {previsto[0]:.2f}")
